 ...
 * it is happening, and turns into a real spinner once the load starts.
 * Sized and placed against the header, not the content, because the
...
function PullSpinner({
  ...
  /* Worked out on the JS side: a worklet cannot call `spacing`, and
     trying to crashes the screen rather than failing quietly. */
...
export function HomeScreen() {
   ...
   * once because four props need to agree about it: the inset, the
   ...
   *
   * React Native's own RefreshControl does not render in this app at
   * all - not on the Feed and not on Room, which also asks for one.
   * Pinning `refreshing` on produced no spinner and did not even
   * displace the content, so the control is never attached; the parity
   * test that claimed we had one only ever checked the source for the
   * word. Measured on the simulator, both screens.
   *
   * So the gesture is read off the scroll we are already following for
   * the header. `pull` is how far past the top the thumb has dragged,
   * which is the only thing the indicator below needs to know.
   */
  ...
  const onScroll = useAnimatedScrollHandler({
    onScroll: (event) => {
      ...
      /* Past the top is a negative offset, and the inset makes the top
         `-headerRoom` rather than zero. */
      pull.value = Math.max(0, -(event.contentOffset.y + headerRoom));
    },
    /* Fired on release. The threshold is the same distance the
       indicator takes to become fully solid, so it commits exactly when
       it looks committed. */
    onEndDrag: () => {
  ...
  return (
    <>
      {/*
       * The spinner, floating under the header.
       *
       * Absolute and OUTSIDE the list, so it does not ride the content
       * and cannot be clipped by it. It fades and grows with the pull,
       * then spins while the load runs - the two states a pull to
       * refresh needs, and the platform control gave neither because it
      ...
      <Animated.ScrollView
         ...
         *
         * It was padding inside the content, and that is why pulling the
         * feed down appeared to do nothing at all. iOS draws the refresh
         * spinner against the scroll view's OWN top edge, above the
         * content - not above the content's padding - so it was landing
         * underneath the floating header every time. Padding cannot move
         * it and `progressViewOffset` does not either: that prop is
         * honoured on Android and ignored here. Verified rather than
         * assumed, by pinning `refreshing` on and finding no spinner at
         * any offset.
         *
         * A content inset moves the scroll view's top edge instead, so
         * the spinner appears below the bar where it can be seen, and
         * the first card still starts clear of it. The matching
         * `contentOffset` is what stops the list opening already
         * scrolled by the size of the inset.
         */
