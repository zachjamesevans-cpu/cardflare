...
describe("the home screen's furniture", () => {
  ...
  it("lets the feed be asked for again, with a spinner that actually draws", () => {
     ...
     * asserted the words "<RefreshControl" were in the file.
     *
     * They were. React Native's RefreshControl does not render in this
     * app at all - not here and not on Room, which also asks for one.
     * Pinning `refreshing` on produced no spinner and did not even
     * displace the content, at any `progressViewOffset`, on a plain
     * ScrollView as well as an Animated one. Measured on the simulator.
     *
     * So the Feed draws its own, off the scroll it already follows for
     * the header. Asserted on the mechanism now rather than on a
     * component name, because a name was exactly what fooled this file.
     */
    expect(app).not.toContain("<RefreshControl");
    expect(app).toContain("function PullSpinner");
    expect(app).toContain("const PULL_TRIGGER");
    /* The gesture: how far past the top, and what to do on release. */
    expect(app).toMatch(
