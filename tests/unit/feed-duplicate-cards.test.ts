...
describe("a Flare posted with no board still reaches your feed", () => {
  ...
  it("reads the boardless ones, by player rather than by session", async () => {
    ...
    /*
     * EVERY author, not just the viewer. Read by player rather than by
     * session, because an area Flare carries no session at all.
     *
     * Reading only the viewer is what made following somebody nearly
     * pointless: their Flares reached you only if they had walked into a
     * room at a shop you had also saved.
     */
