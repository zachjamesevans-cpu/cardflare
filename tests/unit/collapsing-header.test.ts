...
describe("the list makes room for it", () => {
  it("starts its content below the floating bar", () => {
     ...
     * edge, which is what the pull-to-refresh spinner is measured
     * against. With padding the spinner landed under the bar - see
     * below.
     */
  ...
  it("hangs the refresh spinner below the bar, not behind it", () => {
    /*
     * This test used to assert `progressViewOffset`, and passed for as
     * long as the spinner did not exist. React Native's RefreshControl
     * never rendered in this app - no spinner and no content
     * displacement even with `refreshing` pinned true - and that prop is
     * ignored on iOS besides. The Feed draws its own now, placed against
     * the same measurement the inset uses.
     */
    /* The PROP, not the word - the file explains at length why that
       prop is useless here, and the explanation is worth keeping. */
