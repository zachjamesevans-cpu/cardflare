 ...
 *
 * Free of server-only imports so the parsing can be unit-tested and so
 * the browser can count what it is about to send.
 */
...
export type DeckImportState =
  ...
  | {
      status: "saved";
      /** How many cards were added to the want list. */
      saved: number;
      /** Numbers that matched nothing in the catalogue. */
      unknown: string[];
