 ...
 *
 * An ORGANIZER is a player viewer carrying the stores that named them
 * (`organizerStoreIds`), and the event hub is exactly what they were
 * named to run, so they pass here. Nothing about money or membership
 * goes through this file, which is what makes that safe.
 */
...
/**
 * The name a press is stamped with, for the other organizer's screen.
 * A player viewer is their display name; a store login is the email's
 * local part, or "Console" when there is no email to speak of.
 */
async function presserName(): Promise<string | null> {
  const viewer = await getViewer();
  if (viewer.kind === "player") return viewer.playerName;
 ...
 * authorisation check to drift. The computing and writing live in
 * `control.ts`, shared with the phone's remote, so a press means the
 * same thing from a button and from a pocket.
 */
export async function timerControlAction(formData: FormData): Promise<void> {
  ...
  await controlTimer(
    ...
    text(formData, "op"),
    {
      intermissionChoice: text(formData, "intermissionChoice"),
      intermissionCustom: text(formData, "intermissionCustom"),
    },
    await presserName(),
  );
