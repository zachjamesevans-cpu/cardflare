...
export function SignInForm({ next }: { next?: string }) {
  ...
  return (
    ...
    >
      ...
      {/*
       * `link-email` for the DOM id, `email` for the submitted field.
       *
       * This form now shares the sign-in page with the password form, which
       * also has an "Email address" field. Both used `fieldIds("email")`, so
       * the page carried two elements with `id="email"` and two labels
       * pointing at the first of them — clicking this label focused the
       * password form's box instead.
       */}
      <Field name="link-email" label="Email address">
        <TextInput
          {...fieldIds("link-email")}
          name="email"
          type="email"
